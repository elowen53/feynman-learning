...
Load the saved progress and resume from `progress.json.next_action`. Call `feynman_read_coach_memory` before choosing the next remediation strategy, and use only active, evidence-backed coaching patterns. If `progress.json.current_concept_note` exists, load that Markdown note first. If `sessions/` contains a latest session note, use it to recover the exact continuation sentence. Do not restart the topic unless the saved state requires rebuilding.
